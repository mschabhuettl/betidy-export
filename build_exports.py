    ...
    hrows.append(
        {
            ...
            "time": h.get("time") or "",
            "template_id": h.get("templateId") or "",
...
with open("betidy_rooms.csv", "w", newline="", encoding="utf-8-sig") as f:
    w = csv.writer(f)
    w.writerow(["id", "name", "type", "active"])
    for r in rooms:
        w.writerow([r["id"], r["name"], r.get("type", ""), r.get("active")])
with open("betidy_profiles.csv", "w", newline="", encoding="utf-8-sig") as f:
    w = csv.writer(f)
    w.writerow(["id", "name", "active"])
    for p in profiles:
        w.writerow([p["id"], p["name"], p.get("active")])
print(f"betidy_rooms.csv: {len(rooms)} | betidy_profiles.csv: {len(profiles)}")

# ---------- SQLite ----------
if os.path.exists("betidy.sqlite"):
    os.remove("betidy.sqlite")
con = sqlite3.connect("betidy.sqlite")
cur = con.cursor()
...
cur.executemany(f"INSERT INTO tasks VALUES ({','.join('?' * len(COLS))})", [[r[c] for c in COLS] for r in rows])
...
cur.executemany(f"INSERT INTO history VALUES ({','.join('?' * len(HCOLS))})", [[h[c] for c in HCOLS] for h in hrows])
cur.execute("CREATE TABLE rooms(id TEXT PRIMARY KEY, name TEXT, type TEXT, active INT)")
cur.executemany(
    "INSERT INTO rooms VALUES (?,?,?,?)", [(r["id"], r["name"], r.get("type", ""), r.get("active")) for r in rooms]
)
